...
* **`State` (Request Body for POST):** A JSON array of strings representing the sequence of user choices (`id` from the selected item) or text inputs.
...
* **Object within `Prompt` array:** Represents a single item displayed on the watch. Key fields include:
    ...
    * `nextPrompt` (string, optional, enum: `choice`, `text`): Defines the next interaction type. If omitted, it's a final result.
...
2.  **User Interaction (`POST /`)**:
    * If the user selects an item that has `nextPrompt` set to `choice` or `text`:
        * The app sends a `POST /` request to your endpoint.
        * The request body is a JSON array (`State`) containing the sequence of `id`s (for choices) and user-entered text (for text prompts) leading to this point. The `id` or text from the *most recent* interaction is the last element in the array.
        ...
        * Your backend responds with `200 OK` and a *new* JSON `Prompt` array representing the next screen (choices, text input prompt details, or final result).
    * If the user selects an item *without* `nextPrompt` defined:
        * The interaction sequence ends *within the app*. No request is sent to your backend. The `text` of the selected item is simply displayed as the final result.
3.  **Text Input**:
...
def get_main_menu():
    ...
    return [
        ...
        { "id": "status", "text": "Server Status", "icon": "server.rack" }, # Final action
...
def process_state(state):
    ...
    current_step = state[-1] # The ID or text from the last interaction

    # Example: Simple navigation based on the first choice
    if len(state) == 1:
        if current_step == "lights":
            return get_light_rooms()
        elif current_step == "webhook":
        ...
        elif current_step == "secure_note":
             # The user selected "Read Secure Note", the app now shows a text input.
             # The *next* request (POST) will contain the password entered by the user
             # as the last element in 'state'. We don't return anything here,
             # as the app handles showing the text input based on the previous response.
             # However, if the *initial* action itself required text (not common),
             # you would handle it here. We need a line for the app to process
             # after text input, even if it's just a dummy 'next' button.
             return [{"id": "submit_password", "text": "Submit Password"}] # This line won't be displayed, but triggers the POST after text entry.

    # Example: Handling multi-step flows (e.g., Lights -> Room)
    elif len(state) == 2 and state[0] == "lights":
        selected_room = current_step
        return get_lights_in_room(selected_room)

    elif len(state) == 2 and state[0] == "webhook":
    ...
    elif len(state) == 2 and state[0] == "secure_note":
        # The user entered text (password), which is state[1]
    ...
    elif len(state) == 3 and state[0] == "lights":
        room = state[1]
